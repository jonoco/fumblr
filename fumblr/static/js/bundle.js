...
/***/ function(module, exports, __webpack_require__) {
	...
	var _postModal = __webpack_require__(40);

	var _postModal2 = _interopRequireDefault(_postModal);

	var _confirmModal = __webpack_require__(4);

	var _lightbox = __webpack_require__(38);
	...
	(function () {
	    ...
	    var postModal = new _postModal2.default();
...
/***/ function(module, exports) {
	...
	exports.stopScrolling = stopScrolling;
...
/***/ function(module, exports) {
	...
	exports.gotPosts = gotPosts;
	/*
	 * actions types
	 */

	var LOAD_POSTS = exports.LOAD_POSTS = 'LOAD_POSTS';
	var GOT_POSTS = exports.GOT_POSTS = 'GOT_POSTS';
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Comment = function () {
	    ...
	    _createClass(Comment, [{
	        ...
	        value: function openCommentModal(e) {
	        ...
	        value: function sendComment(e) {
	            ...
	            var text = this.$form.find('.comment-text').val();

	            _axios2.default.post(url, {
	                text: text
	            }).then(function (res) {
	                if (res.data.comment) {
	                    _this.$cmtModal.modal('hide');
	                } else {
	                    document.location.assign(res.request.responseURL);
	                }
	            }).catch(function (err) {
	        ...
	        value: function deleteComment(e) {
	            var cmtID = $(e.currentTarget).data('id');
	            console.log('delete ' + cmtID);

	            (0, _confirmModal.askConfirm)({ title: 'Delete comment?', btn: 'Delete' }).then(function () {
	                _axios2.default.post('/comment/delete/' + cmtID).then(function (res) {
	                    if (res.data.comment) {
	        ...
	        key: 'toggleCommentControls',
	        value: function toggleCommentControls(e) {
	            var $comment = $(e.currentTarget);
	            var cmtID = $comment.data('id');
	...
	var LightBox = function () {
		...
		_createClass(LightBox, [{
			...
			value: function hideLightbox() {
				(0, _utils.stopScrolling)(false);
				this.$lightbox.addClass('hide');
			}
		}, {
			key: 'showLightbox',
			value: function showLightbox() {
				(0, _utils.stopScrolling)(true);
	...
	var PostModal = function () {
	    function PostModal() {
	        ...
	        this.$file.change(function (e) {
	            _this.handleFiles(_this.$file[0].files);
	        });
	        ...
	        this.$dropbox.on('dragenter', this.dragenter.bind(this)).on('dragover', this.dragover.bind(this)).on('dragleave', this.dragleave.bind(this)).on('drop', this.drop.bind(this));
	    ...
	    _createClass(PostModal, [{
	        key: 'initUploadModal',
	        value: function initUploadModal() {
	            this.clearModal();
	        ...
	        key: 'initEditModal',
	        value: function initEditModal(post) {
	            var _this2 = this;
	            ...
	            post.images.forEach(function (image) {
	                _this2.addImage(image.link, 'image-' + image.id);
	                _this2.formData['image-' + image.id] = '' + image.id;
	            });
	        ...
	        value: function submit() {
	            ...
	            this.showLoading();
	            var $progress = this.$postModal.find('.progress');

	            var formData = new FormData();
	            formData.append('text', this.$text.val());
	            formData.append('tags', this.$tags.val());

	            _lodash2.default.forIn(this.formData, function (v, k) {
	                formData.append(k, v);
	            });

	            var url = this.$postForm.data('action');
	            _axios2.default.post(url, formData, {
	                onUploadProgress: function onUploadProgress(pe) {
	                    var progress = Math.floor(pe.loaded / pe.total * 100);
	                    $progress.text(progress + '%');
	                }
	            }).then(function (res) {
	                _this3.$postForm.removeClass('is-uploading');
	                _this3.hideLoading();
	                if (res.data.reload) {
	                    document.location.reload(true);
	        ...
	        value: function handleFiles(files) {
	            ...
	            $.each(files, function (i, f) {
	                var imageID = (0, _utils.hashCode)(f.name);
	                var reader = new FileReader();
	                reader.onload = function (e) {
	                    _this4.addImage(e.target.result, imageID);
	                };

	                reader.readAsDataURL(f);
	                _this4.formData['' + imageID] = f;
	            });
	        ...
	        value: function uploadImageURL(e) {
	            ...
	            this.$url.val('');
	            this.showLoading();
	            _axios2.default.post('/image/url', {
	                url: url
	            }).then(function (res) {
	                var image = res.data.image;
	                _this5.addImage(image.link, 'image-' + image.id);
	                _this5.formData['image-' + image.id] = '' + image.id;
	                _this5.hideLoading();
	            }).catch(function (err) {
	        ...
	        key: 'addImage',
	        value: function addImage(image, id) {
	        ...
	        value: function removeImage(e) {
	            ...
	            _lodash2.default.unset(this.formData, '' + imageID);
	        ...
	        key: 'getEditPost',
	        value: function getEditPost(e) {
	            var _this6 = this;

	            var postID = $(e.currentTarget).data('post');

	            _axios2.default.get('/post/edit/' + postID).then(function (res) {
	                var post = JSON.parse(res.data.post);
	                _this6.initEditModal(post);
	            }).catch(function (err) {
	                console.log(err);
	            });
	        }
...
/***/ function(module, exports, __webpack_require__) {
	...
	function pages() {
		var state = arguments.length <= 0 || arguments[0] === undefined ? pagesInitialState : arguments[0];
	...
	var fumblrApp = (0, _redux.combineReducers)({
		pages: pages
	});

	exports.default = fumblrApp;
